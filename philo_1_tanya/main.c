...
/*   Updated: 2024/01/27 20:44:34 by mnurlybe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	output_message(t_philo *philo, bool died, t_status state)
{
	...
	if (state == DIED)
		printf("%ld %d %s\n", ms_time() - philo->table->start_time,
			philo->id + 1, "\033[1;5;91;40m died");
	else if (state == EATING)
		printf("%ld %d %s\n", ms_time() - philo->table->start_time,
			philo->id + 1, "\033[92mis eating");
	else if (state == SLEEPING)
		printf("%ld %d %s\n", ms_time() - philo->table->start_time,
			philo->id + 1, "\033[36mis sleeping");
	else if (state == THINKING)
		printf("%ld %d %s\n", ms_time() - philo->table->start_time,
			philo->id + 1, "\033[95mis thinking");
	else if (state == TOOK_FORK_1 || state == TOOK_FORK_2)
		printf("%ld %d %s\n", ms_time() - philo->table->start_time,
			philo->id + 1, "\033[90mhas taken a fork");
	pthread_mutex_unlock(&philo->table->output_mutex);
}
