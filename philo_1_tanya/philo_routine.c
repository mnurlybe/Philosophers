...
/*   Updated: 2024/01/25 19:31:37 by mnurlybe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*philosopher(void *table_data)
{
	...
	if (philo->table->must_eat_count == 0) // it means that no philosophers should eat at all and simulation stops here; 
		return (NULL);
	pthread_mutex_lock(&philo->meal_time_mutex);
	philo->last_meal = philo->table->start_time;
	pthread_mutex_unlock(&philo->meal_time_mutex);
	while (ms_time() < philo->table->start_time) // ????? Don't understand what it means
		continue ;
